...
- Antigravity 的 `AG-002`、`AG-003` 已验收。`AG-004` 仅允许它在 `.ai/TASKS.md` 精确列出的隔离 worktree、真实发布和测试范围内工作；主工作区的产品源码仍冻结。
...
- 其他 AI 只能阅读，不得写任何文件。
- 除 `AG-004` 明确列出的隔离工作区文件外，任何代理不得修改 `ios/`、后端、部署配置、主分支或 Git 历史。`codex/ios-api-base-integration` 的 AG-003 集成已通过复验并合入本地主分支；未经 Codex 再次验收不得合入任何后续源码。

...
- Codex 可在独立 `worktrees/codex-v3-publish-profile` 执行 `CX-003`，只可修改 `PublishView.swift`、`ProfileView.swift`、交接和群聊；它不得触碰 `AG-006` 的 Progress/Track/Delivery/Content 路径、任何 ViewModel 或业务语义。
...
- 其他 AI 只能阅读，不得写任何文件。
- 除 `AG-006` 和 `CX-003` 各自隔离 worktree 的精确授权路径外，任何代理不得修改 `ios/`、后端、部署配置、主分支或 Git 历史。`codex/ios-api-base-integration`、`codex/ios-publish-integration`、`codex/ios-response-integration`、`codex/v3-home-foundations`、`codex/v3-nearby-response` 已分别把 AG-003/AG-004/AG-005/CX-001/CX-002 复验并合入本地主分支；未经 Codex 再次验收不得合入任何后续源码。

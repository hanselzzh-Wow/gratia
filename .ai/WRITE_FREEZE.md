...
- Antigravity 的 `AG-002`、`AG-003`、`AG-004` 已验收且 AG-004 已合入本地主分支。`AG-005` 的外部写权限已收回；Codex 已在受控集成 worktree 独立验收通过，现只等待合入本地 `main`。`AG-006` 仍为 `PLANNED`，在 Codex 从最新 main 正式派发前，所有外部成员只读，主工作区源码仍冻结。
...
- 其他 AI 只能阅读，不得写任何文件。
- 除 Codex 的受控 `AG-005` 集成及未来正式派发的独立任务外，任何代理不得修改 `ios/`、后端、部署配置、主分支或 Git 历史。`codex/ios-api-base-integration`、`codex/ios-publish-integration` 已分别把 AG-003/AG-004 复验并合入本地主分支；未经 Codex 再次验收不得合入任何后续源码。

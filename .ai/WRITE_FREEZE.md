...
- Codex 正在隔离 `worktrees/codex-v3-nearby-response` 执行 `CX-002`，只可修改 `NearbyView.swift`、交接与群聊。它不得触碰 `AG-006` 的 Progress/Track/Delivery/Content 路径，也不得改已验收的 response ViewModel 行为。
...
- 其他 AI 只能阅读，不得写任何文件。
- 除 `AG-006` 和 `CX-002` 各自隔离 worktree 的精确授权路径外，任何代理不得修改 `ios/`、后端、部署配置、主分支或 Git 历史。`codex/ios-api-base-integration`、`codex/ios-publish-integration`、`codex/ios-response-integration`、`codex/v3-home-foundations` 已分别把 AG-003/AG-004/AG-005/CX-001 复验并合入本地主分支；未经 Codex 再次验收不得合入任何后续源码。

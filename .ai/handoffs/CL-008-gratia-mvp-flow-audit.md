...
## 8. 补充发现（同基线独立二次通读，2026-07-19 晚追加）

以下为对 `463a420` 的第二轮逐行独立审计中发现、且上文未覆盖的断点。原有章节文字未做任何改动；编号沿用上文体系另立"S"前缀，优先级判断供 Codex 合并裁量。

**S-P0-1（业务闭环缺口，需产品裁决）**：`delivered → completed` 的"发布者确认完成"没有任何 App 内动作。查询进度页对 `delivered` 仅显示「待确认」（`ProgressView.swift:23-25` 的 `statusText` 映射），交付预览看完即止；`README.md` 产品闭环承诺的"发布者确认完成"环节在客户端不存在，最后一步只能由运营线下代办。需产品负责人裁决：MVP 阶段是否接受运营代确认——若接受，查询结果页应补一句诚实说明（如"确认完成暂由平台协助处理"）；若不接受，需另立含后端接口的实现任务，不宜混入第 5 节的首张切片。

**S-P1-1（语义冲突）**：发布成功页「状态流转路线」写"已提交 → 审核中 → 匹配中 → 已接单"（`PublishView.swift:398-421`），与 Core 冻结状态文案"待审核/待匹配/已派单"（`GratiaCore/Models.swift:21-28`）不一致，且"审核中"被渲染为已完成态。用户拿编号去查询时会看到另一套词，两页对不上。修复属纯文案，可并入第 5 节切片的允许文件。

**S-P1-2（校验断裂）**：发布流步内校验与提交校验不一致——第 1 步仅要求地标非空（提交要求 2-40 字），第 2 步仅要求非空且 ≤120 字（提交要求 ≥5 字），见 `PublishView.swift:17-31` 与 `PublishWishViewModel.validate`。用户可携非法值走到第 3 步，提交失败后字段红字落在第 1/2 步视图内，当前步只见一句"输入校验未通过"，须自行回翻查找。

**S-P1-3（死 UI）**：「我的」页「消息通知设置」行未包在任何 Button 内（`ProfileView.swift:111`），外观与上两行可点行一致且尾注"去系统设置"，实际点按无反应、也未真跳系统设置。

**S-P1-4（文案过诺）**：「我帮助的」文案"响应经运营确认后，完成记录会出现在这里"（`ProfileView.swift:263`）在无账户机制下无法兑现，建议改为与「我发布的」一致的编号查询口径。

**S-P1-5（无障碍补充，与上文 P1-4 同页合修）**：`DeliveryPreviewView` 关闭按钮（xmark）无 `accessibilityLabel`（`DeliveryPreviewView.swift:21-28`）；首页导航栏品牌标题整体 `accessibilityHidden(true)`（`HomeView.swift:36`），VoiceOver 在首页读不到任何页面标题，仅靠 Dock 项名兜底。

**并入建议**：S-P1-1/S-P1-5 可直接并入第 5 节首张切片（不扩允许文件清单之外的范围）；S-P0-1 与 S-P1-2/S-P1-3/S-P1-4 各需产品或任务边界裁决，建议单列。本补充不改变第 3 节 `StoryFeedSource` 裁定与第 4 节隐私专项结论（二次通读同样未发现隐私 P0）。

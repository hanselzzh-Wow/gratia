...
- 产品负责人新增冻结：发布进度区只删右侧“地点”两字；登录升为 P0，游客可浏览，但新发布/响应最终提交必须 Apple 登录，不保留匿名提交降级；服务端用户/会话/数据归属并保留编号+联系方式历史找回/兼容查询；通知设置行必须直达本 App 系统通知设置；审核后台仍独立且 App 要讲清审核通过后才进入帮助页。
- `CL-009` 设计交接 `4085b85` 已 ACCEPTED；Codex 覆盖其“暂不登录继续提交”的默认，冻结为直接 Apple 登录。`AG-012` 初次实际 HEAD `814209c` 的 App 结果包为 25/25、0 failure/skip/runtime warning，但因漏 Dock VoiceOver、36pt CTA、弱测试、缺截图、`diff --check` 失败及三套 commit 号不一致被退回原范围修订，尚未合入 1.0 演进线。
...
1. Antigravity 只在原 `AG-012` worktree 修订被退回项；Codex 按截图、真实系统跳转、25+ 测试与提交一致性独立复验，通过后再集成到从 1.0 演进的产品线。
2. Codex 依据已验收 `CL-009` 和“新写入必须登录”裁决冻结 Worker/D1/Apple token/会话/归属/删除架构；在 AG-012 收口后向 Antigravity 派发第一张跨层登录任务。
3. 账号接通后补齐“我发布的/我帮助的”服务端归属与发布者确认完成，再按真实发布→审核→帮助响应→派单交付→完成做模拟器/真机 E2E。

...
状态：`FROZEN / CL-003 ACCEPTED / v3.1 navigation amendment`
...
- **默认阴影**：零阴影。唯一例外是详情响应 Bottom Sheet 在半展开/拖拽时使用 `0 2px 8px rgba(23,19,16,0.08)`，不得扩散到普通卡片或按钮。iOS 26 系统 Liquid Glass 的平台材质/高光不属于自定义阴影例外。
- **导航与内容**：坚持五栏原生 Tab Bar；产品 chrome 保持低彩度，用户上传照片/视频才是最丰富的视觉内容。

### v3.1 导航平台修订（2026-07-18）

在 iOS 26 及以上版本，使用 SwiftUI 系统 `TabView`/原生 Tab Bar 提供的**浮动 Liquid Glass Dock**；让系统处理毛玻璃、浮动、滚动时收缩、对比度和“降低透明度”等适配。其悬浮特性用于让首页内容流从底部自然延伸，提升沉浸感与可见内容面积。iOS 16–25 继续使用同一套原生 Tab Bar 的系统默认外观。

这不是授权制作网页式自定义 Dock：不得手工固定 blur、强行绘制胶囊阴影，或重新实现 Tab 的点击/无障碍行为。首页、附近、发布、进度、我的五个目的地必须保留系统可访问名称、44pt 最小点击区和 VoiceOver 语义；是否在非选中状态持续显示文字由系统平台样式决定，不手工强制。

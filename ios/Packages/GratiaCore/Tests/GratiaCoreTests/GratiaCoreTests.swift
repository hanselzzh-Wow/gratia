...
// MARK: - Test Cases
struct GratiaCoreTests {

    let baseURL = URL(string: "https://test.gratia.com")!
    ...
    @Test func testValidationError400() async throws {
        ...
        } catch let GratiaAPIError.badRequest(message, fields) {
    ...
    @Test func testResponseConflict409() async throws {
        ...
        } catch let GratiaAPIError.conflict(message) {
    ...
    @Test func testTrackWishNotFound404() async throws {
        ...
        } catch let GratiaAPIError.notFound(message) {
            #expect(message == "没有找到匹配的心愿")
            let errDesc = GratiaAPIError.notFound(message: message).localizedDescription
    ...
    @Test func testRateLimit429() async throws {
        ...
        do {
            _ = try await client.trackWish(request: request)
        } catch let GratiaAPIError.rateLimited(retryAfterSeconds) {
    ...
    @Test func testCorruptedJson() async throws {
        ...
        } catch GratiaAPIError.decodingError {
            // Success
        } catch {
    ...
    @Test func testRequestCancelled() async throws {
        ...
        let transport = MockTransport(onStart: {
            ...
            for _ in 0..<100 {
                if Task.isCancelled {
                    throw GratiaAPIError.requestCancelled
                }
                try await Task.sleep(nanoseconds: 5_000_000) // 5ms sleep
            }
            throw GratiaAPIError.serverError(message: "Should have been cancelled")
        }
    ...
    @Test func testDeliverableUrlPrivacy() async throws {
        let secretUrl = "https://gratia.com/api/deliverables/uuid?token=SECRET_TOKEN_123"
        let transport = MockTransport { _ in
            throw GratiaAPIError.networkError(NSError(domain: "NSURLErrorDomain", code: -1009, userInfo: [NSURLErrorFailingURLErrorKey: URL(string: secretUrl)!]))
        }
        ...
        do {
            _ = try await client.listWishes(city: nil)
        } catch let GratiaAPIError.networkError(underlying) {
            let desc = underlying.localizedDescription
            let apiErrDesc = GratiaAPIError.networkError(underlying).localizedDescription
    ...
    @Test func testTrackWishSuccessFullDecoding() async throws {
        let wishJson = """
        {
            "wish": {
                ...
                "deliverable": {
                    "id": "deliv-999",
                    "kind": "spoken_video",
                    "url": "https://gratia.com/api/deliverables/deliv-999?token=SECRET_123",
                    "note": "录制好了，请查收",
        ...
        #expect(wish.deliverable?.url.absoluteString == "https://gratia.com/api/deliverables/deliv-999?token=SECRET_123")
